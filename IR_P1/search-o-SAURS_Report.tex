...
\node[data, below=0.8cm of index, xshift=-3cm] (query) {Boolean Query};
...
Three scripts: \texttt{search-o-SAURS\_preprocess.py} (454 lines),
\texttt{search-o-SAURS\_indexer.py} (69 lines),
\texttt{search-o-SAURS\_search.py} (434 lines).
...
The test suite (\texttt{tests/test\_search.py}, 720 lines) contains
\textbf{220 tests} across \textbf{12 groups}.  All 220 tests pass.

\begin{table}[H]
\centering\small
\caption{Test suite: 220 tests across 12 groups.}
\label{tab:tests}
\begin{tabular}{@{}c l c p{5.5cm}@{}}
\toprule
\textbf{\#} & \textbf{Group} & \textbf{Tests} & \textbf{What It Validates} \\
\midrule
1 & Tokenization & 15 & Abbreviations, possessives, hyphens, slashes, number-word splits, noise \\
2 & Normalization & 25 & All 5 patterns, all exception guards, case folding \\
3 & Stop Words & 4 & Removal correctness, edge cases \\
4 & Stemming & 9 & Porter stemmer on key corpus terms \\
5 & Deduplication & 4 & Order preservation, identity \\
6 & Query$\leftrightarrow$Doc Parity & 26 & Exact normalization+stemming match \\
7 & Index Correctness & 8 & Header, sort, df accuracy, no duplicates \\
8 & Binary Search & 55 & Stratified sample + boundary + nonexistent terms \\
9 & AND/OR Merges & 24 & Gallop unit tests, set operations, edge cases \\
10 & End-to-End & 13 & Raw queries $\to$ correct results \\
11 & Spot Checks & 8 & Document membership, commutativity \\
12 & Invariants & 30 & $|A \cap B| \leq \min(|A|,|B|)$, inclusion-exclusion \\
\midrule
...
Test suite & 220 tests, 12 groups \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Prerequisite}: Python 3.8+ (no external libraries).

\begin{lstlisting}[style=shellstyle,numbers=none]
# One-command execution (runs all 4 steps):
chmod +x run_pipeline.sh && ./run_pipeline.sh

# Or step-by-step:
python search-o-SAURS_preprocess.py           # Step 1: Preprocess
python search-o-SAURS_indexer.py              # Step 2: Build index
python search-o-SAURS_search.py "aerodynamic AND experimental" \
    output/results/output.txt                 # Step 3: Query
python tests/test_search.py                   # Step 4: Tests
\end{lstlisting}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Search}: File-based binary search (O($\log V$) per term);
        ...
        skewed lists).
  \item \textbf{Testing}: 220 tests across 12 groups verify correctness from
...
\midrule
\texttt{search-o-SAURS\_preprocess.py} & 454 & Preprocessing pipeline \\
\texttt{search-o-SAURS\_indexer.py} & 69 & Inverted index builder \\
\texttt{search-o-SAURS\_search.py} & 434 & Boolean search engine \\
\texttt{porter.py} & 367 & Porter Stemmer (canonical) \\
\texttt{tests/test\_search.py} & 720 & Comprehensive test suite (220 tests) \\
\texttt{run\_pipeline.sh} & 22 & One-command pipeline runner \\
\bottomrule
\end{tabular}
\end{table}
